          ...
          <el-collapse-item v-for="(item, index) in uploadFileList" :key="index">
            <template >
              <div class="upload-file-item">
                ...
                <div class="file-info-item file-size">文件大小：{{ item.size }}</div>
      ...
      methods: {
        handleUpload() {
          ...
          this.getFileMd5(currentFile.raw, async (md5) => {
            ...
            // self.$set( 'chunkList', currentFile)
          ...
          return new Promise((resolve) => {
            const handler = () => {
              if (chunkList.length) {
                ...
                this.$axios.put(chunkItem.uploadUrl, chunkItem.chunk.file, {
                  // 上传进度处理
        ...
        getFileUploadUrls(fileParam) {
          let url = `/api/getUrl?_=${Math.random()}`
          return this.$axios.post(url, fileParam)
        },
        saveFileUploadId(data) {
          localStorage.setItem(FILE_UPLOAD_ID_KEY, data)
        },
        checkFileUploadedByMd5(md5) {
          let url = `/api/check?md5=${md5}`
        ...
        mergeFile(file) {
          const self = this
          let url = `/api/merges?uploadId=${file.uploadId}&filename=${file.fileName}&hash=${file.md5}`
          return new Promise((resolve, reject) => {
